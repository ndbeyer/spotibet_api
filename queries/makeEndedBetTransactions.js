...
const getEndedBets = async input => {
  ...
  return (
    await db.query(
      `SELECT * FROM public.bet WHERE transactions = false AND (now() at time zone 'utc') > end_date ${
        input ? "AND id = ANY($1)" : ""
      }`,
      input ? [ids] : []
    )
...
const makeEndedBetTransactions = async (input) => {
  try {
    ...
    for (const artistId in betsPerArtist) {
      ...
      await Promise.all(
        bets.map(async bet => {
          const endDate = bet.end_date.toISOString().substr(0, 10);
          const endDateEntry =
            data &&
            data.payload &&
            data.payload.filter(
          ...
          const cashPerWinnerAmount = lostMoneyToDistribute / allWinnerAmounts;
          ...
          await Promise.all(
            winningParticipants.map(async ({ amount, user_id }) => {
              const addedCash =
                Number(amount) + Number(amount) * Number(cashPerWinnerAmount);
              await Promise.all([
                await db.query(
